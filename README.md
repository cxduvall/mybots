...
To isolate the variable of interest, a "PID" or "NOPID" flag is taken on the command line for main.py, which is then passed down into search.py -> PARALLEH_HILL_CLIMBER -> SOLUTION (for neural computations) -> simulate.py -> SIMULATION -> ROBOT (for simulation purposes).
